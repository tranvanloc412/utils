...
STANDARD_EXCLUDE = (
    TAG_PRESETS["nef2"] + TAG_PRESETS["nabserv"] + 
    TAG_PRESETS["cps"] + TAG_PRESETS["wiz"]
)
...
SERVICE_CONFIGS = {
    ...
    "ebs": ServiceConfig(resource_type="ec2:volume"),
...
class TagMatcher:
    ...
    def __init__(self, include_rules: List[Dict] = None, exclude_rules: List[Dict] = None):
...
class ResourceScanner:
    ...
    def scan_all_resources(self) -> List[Tuple[str, Dict[str, str]]]:
        ...
        tagging_api_services = {
            ...
            if v.api_method == "tagging_api"
        }
        if tagging_api_services:
            resource_types = [config.resource_type for config in tagging_api_services.values()]
            logger.info(f"🔍 Scanning via Tagging API: {len(resource_types)} resource types")
            matched.extend(self._scan_with_tagging_api(resource_types, tagging_api_services))
        
        # Scan via direct APIs
        direct_api_services = {
            k: v for k, v in SERVICE_CONFIGS.items() 
            if v.api_method == "direct"
        }
    ...
    def _scan_with_tagging_api(self, resource_types: List[str], 
                              service_configs: Dict[str, ServiceConfig]) -> List[Tuple[str, Dict[str, str]]]:
        ...
        for page in paginator.paginate(ResourceTypeFilters=resource_types):
            for resource in page.get("ResourceTagMappingList", []):
                ...
                rtype = resource.get("ResourceType") or self.arn_parser.extract_resource_type(arn)
                
                # Find matching service config
                service_name = self._find_service_by_resource_type(rtype, service_configs)
                if not service_name:
    ...
    def _scan_with_direct_api(self, service_name: str, 
                             config: ServiceConfig) -> List[Tuple[str, Dict[str, str]]]:
    ...
    def _scan_iam_roles(self, config: ServiceConfig) -> List[Tuple[str, Dict[str, str]]]:
        ...
        for page in paginator.paginate():
            for role in page.get("Roles", []):
                try:
                    arn = role["Arn"]
                    response = client.list_role_tags(RoleName=role["RoleName"])
                    ...
                    logger.debug(f"⚠️ Error processing IAM role {role['RoleName']}: {e}")
        
        return matched
    
    def _scan_route53_zones(self, config: ServiceConfig) -> List[Tuple[str, Dict[str, str]]]:
    ...
    def _find_service_by_resource_type(self, resource_type: str, 
                                      service_configs: Dict[str, ServiceConfig]) -> Optional[str]:
...
class ResourceTagger:
    ...
    def tag_resources_with_nis_managed(self, matched_resources: List[Tuple[str, Dict[str, str]]]) -> Dict[str, int]:
        """Add 'nis_managed=true' tag to matched resources"""
        client = self.session.client("resourcegroupstaggingapi")
        stats = {"success": 0, "failed": 0, "errors": []}
        
        logger.info(f"🏷️ Tagging {len(matched_resources)} resources with nis_managed=true")
        ...
        logger.info(f"🏷️ Tagging complete - Success: {stats['success']}, Failed: {stats['failed']}")
        return stats


def write_results_to_csv(lz_name: str, matched_resources: List[Tuple[str, Dict[str, str]]]):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Simplified AWS resource tag scanner"
    )
    parser.add_argument(
        "--landing-zones", "-l", nargs="*", default=[],
        help="Landing zone names (e.g., cmsnonprod appanonprod)"
    )
    parser.add_argument(
        "--environment", "-e", default="nonprod",
        choices=["prod", "nonprod", "preprod"],
        help="Environment suffix to filter zones"
    )
    parser.add_argument(
        "--test", "-t", action="store_true",
        help="Use test account configuration"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tag-nis-managed", action="store_true",
    ...
    if args.test:
        ...
        try:
            ...
            if args.tag_nis_managed and matched:
                ...
                logger.info(f"🏷️ Tagged - Success: {tag_stats['success']}, Failed: {tag_stats['failed']}")
        ...
        try:
            ...
            if args.landing_zones:
                zones = [z for z in zones if any(z.split()[1] == lz for lz in args.landing_zones)]
            else:
            ...
            if args.tag_nis_managed:
                logger.info(f"🏷️ Tagging summary - Success: {total_tagged_success}, Failed: {total_tagged_failed}")
